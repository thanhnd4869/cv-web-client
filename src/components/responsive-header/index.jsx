...
import ResponsiveSubMenu from './responsive-sub-menu';
import ResponsiveMenuData from './responsive-sub-menu/ResponsiveMenuData';
...
const ResponsiveHeader = () => {
  ...
  return (
    <div className='z-[9999] block lg:hidden w-full py-4'>
      ...
      >
        <span className='mirror absolute right-4 top-4' onClick={showResponsiveSideMenu}>
          <CgClose className='text-white cursor-pointer hover:text-accent' size={20} />
        </span>
        <ul
        ...
        >
          {ResponsiveMenuData.map((item, index) => (
            <ResponsiveSubMenu item={item} key={index} />
          ))}
        </ul>
      </div>
      {/* Responsive Header */}
